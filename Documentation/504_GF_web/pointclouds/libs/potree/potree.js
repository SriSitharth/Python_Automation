...
//TODO: PointCab set motion and project information here
Potree.ProjectInfo = class ProjectInfo {

	constructor() {
		this.motionProj = new THREE.Matrix4();
		this.readProjectInformation();
	}


	readProjectInformation() {
		// url to data file directly
		let url = "data.json";

		$.getJSON(url, (data) => {
			if (data.projectMotion)
				this.motionProj.fromArray(data.projectMotion);
		});
	}

	// Return the Motion of the complete project as THREE.Matrix4
	getMotionProj() {
		// No need for check due to identity when initializing
		return this.motionProj;
	}
};

Potree.Motion = new Potree.ProjectInfo();
Potree.pointBudget = 1 * 1000 * 1000;
Potree.scanStations = -1;
Potree.framenumber = 0;

Potree.setScanStations = function (scanStations) {
	Potree.scanStations = scanStations;
}
...
//TODO: check if this works or delete it
Potree.PointCab2SceneConnector = function (name, position, lookAt) {
	let scenePC = new Potree.Scene();
	//Potree.Viewer.setTopView();
	//window.viewer = new Potree.Viewer(document.getElementById("potree_render_area"));
	//viewer.setTopView();
	//window.alert(Potree.Viewer.getPointBudget());
	//viewer.setTopView();
	scenePC.view.position.set(position.x, position.y, position.z);
	scenePC.view.lookAt(new THREE.Vector3(lookAt.x, lookAt.y, lookAt.z));
	//window.alert("PointCab2SceneConnector");
	//	Potree.Scene.scene.view.position.set(position.x, position.y, position.z);
	//	Potree.Scene.scene.view.lookAt(new THREE.Vector3(lookAt.x, lookAt.y, lookAt.z));
	return false;
};
		...
		xhr.onreadystatechange = function () {
			if (xhr.readyState === 4 && (xhr.status === 200 || xhr.status === 0)) {
				...
				{ // load root
					...
					pco.root = root;
					//TODO: Check this if this is helping
					//if(loadPC)
					//{
					pco.root.load();
					//}
...
Potree.Measure = class Measure extends THREE.Object3D {
	constructor() {
		...
		// Fixed: Martin multiplier for units
		this.lengthUnit = { code: "m", multi: 1.0000 };
	...
	update() {
		//Fixed: Martin multiplier for units in measurement elements
		//FIXED: PointCab add motion here
		if (this.points.length === 0) {
			return;
			...
			{// coordinate labels
				...
				let pWorld = (new THREE.Vector4(position.x, position.y, position.z, 1.0)).applyMatrix4(Potree.Motion.getMotionProj());

				let msg = ((pWorld.x * this.lengthUnit.multi).toFixed(3) + " | " + (pWorld.y * this.lengthUnit.multi).toFixed(3) + " | " + (pWorld.z * this.lengthUnit.multi).toFixed(3) + " [" + this.lengthUnit.code + "]");
		...
		for (let i = 0; i <= lastIndex; i++) {
			...
			{// edge labels
				...
				edgeLabel.setText((distance * this.lengthUnit.multi).toFixed(3) + " [" + this.lengthUnit.code + "]");
			...
			{// angle labels
				...
				let msg = (angle * (180.0 / Math.PI)).toFixed(2) + "[" + '\u00B0' + "]";
			...
			{// coordinate labels
				...
				let msg = point.position.z.toFixed(3) + " [" + this.lengthUnit.code + "]";
				coordinateLabel.setText(msg);
		...
		{ // update height stuff
			...
			if (this.showHeight) {
				...
				let msg = ((height * this.lengthUnit.multi).toFixed(3)) + " [" + this.lengthUnit.code + "]";
		...
		{ // update area label
			...
			let area2unit = ((Math.sqrt(this.getArea())) * this.lengthUnit.multi) * ((Math.sqrt(this.getArea())) * this.lengthUnit.multi);
			//let area2unit=Math.sqrt(this.getArea());
			let msg = area2unit.toFixed(3) + " [" + this.lengthUnit.code + "\u00B2" + "]";
		...
		for (let measure of measurements) {
			...
			for (let j = 0; j < measure.coordinateLabels.length; j++) {
				...
				labelPos = new THREE.Vector3().addVectors(
					...
					*/
				// set label position at location of point, no mirror error effect when camera in point cloud
				let labelPos = new THREE.Vector3(measure.points[0].position.x, measure.points[0].position.y, measure.points[0].position.z + (distance / 30 * camera.fov * Math.PI / 180));
				label.position.copy(labelPos);

				let pr = Potree.utils.projectedRadius(1, camera.fov * Math.PI / 180, distance, domElement.clientHeight);//6,316
...
Potree.GeoJSONExporter = class GeoJSONExporter {
	...
	static measurementToFeatures(measurement) {
		...
		for (i = 0; i < coords.length; i++) {
			...
			let pWorld = (new THREE.Vector4(coords[i][0], coords[i][1], coords[i][2], 1.0)).applyMatrix4(Potree.Motion.getMotionProj());
			let coordsGlob = [];
			pWorld.toArray(coordsGlob);
			coords[i] = coordsGlob;
		}
...
Potree.Scene = class extends THREE.EventDispatcher {
	...
	isLoaded2Scene(pointcloud) {
		return this.loaded2Scene[pointcloud.name];
	};


	makePointCloudVisible(pointcloud) {
		// remove inivisble point cloud from array and add to scene
		for (i = 0; i < this.invisiblePC.length; i++) {
			if (this.invisiblePC[i].name == pointcloud.name) {
				this.scenePointCloud.add(this.invisiblePC[i]);
				this.invisiblePC[i].visible = true;
				this.loaded2Scene[pointcloud.name] = true;
				this.invisiblePC.splice(i, 1);
			}
	...
	addPointCloud(pointcloud) {
		...
		{
			this.invisiblePC.push(pointcloud);
			this.loaded2Scene[pointcloud.name]=false;
		}*/
		//else
		{
...
Potree.Viewer = class PotreeViewer extends THREE.EventDispatcher {
	...
	constructor(domElement, args) {
		...
		this.moveSpeed = 10;
		//Fixed: Martin recalculate units from meter function
		this.LENGTH_UNITS = {
			METER: { code: "m", multi: 1.0000 },
			FEET: { code: "ft", multi: 0.3048 },
			INCH: { code: "\u2033", multi: 0.0254 }
		};
	...
	initThree() {
		...
		this.skybox = Potree.utils.loadSkybox(new URL(Potree.resourcePath + "/textures/skyboxpc/").href);
...
Potree.MapView = class {
	...
	init() {
		...
		this.onPointcloudAdded = e => {
			...
			if (Potree.scanStations == this.viewer.scene.pointclouds.length) {//Fixed: Martin initSceneList only once after all scans are loaded; 
				console.log("loadPointCloud and initis cene list");
				initSceneList();
				console.log("Finished intiscenelist");
			}
		};
	...
	setScene(scene) {
		...
		for (let pointcloud of this.viewer.scene.pointclouds) {
			...
			//Fixed: Martin initSceneList only once after all scans are loaded; 
			if (Potree.scanStations == this.viewer.scene.pointclouds.length) {
				consol.log("loadPointCloud and initis cene list");
				initSceneList;
			}
		}
...
function initMeasurementDetails() {
	...
	let createCoordinatesTable = (measurement) => {
		...
		for (let point of measurement.points) {
			...
			// TODO: PointCab insert Motion here?
			let pWorld = (new THREE.Vector4(position.x, position.y, position.z, 1.0)).applyMatrix4(Potree.Motion.getMotionProj());

			let x = Potree.utils.addCommas(pWorld.x.toFixed(3));
			let y = Potree.utils.addCommas(pWorld.y.toFixed(3));
			let z = Potree.utils.addCommas(pWorld.z.toFixed(3));
	...
	class AnglePanel extends MeasurePanel {
		constructor(scene, measurement) {
			...
			this.icon = Potree.resourcePath + "/icons/Angle.svg";
...
function initSceneList() {
	...
	let initUIElements = function (i) {
		...
		inputVis.click(function (event) {
			if (!viewer.scene.isLoaded2Scene(pointcloud)) {
				viewer.scene.makePointCloudVisible(pointcloud);
			}
	...
	// Fixed: Mg we don't change the scene ever so unneccessary
	/*
		viewer.addEventListener("scene_changed", (e) => {
			buildSceneList();
	...
	*/
	// Info: Will not affect the performance anymore because all point clouds are added already.
